...
function determineInternshipPotential(offered: string, scale: string): string {
  if (offered) {
...
interface FirmData {
  ...
  internshipOffered: string | null;
  website: string | null;
  officialEmail: string | null;
  applicationMethod: string | null;
  portfolioEmphasis: string | null;
...
function importFirms() {
  ...
  for (const line of oldLines) {
    ...
    let principalArchitect: string | null = null;
  ...
  const stats = {
    ...
    byInternship: {} as Record<string, number>,
    highInternship: 0,
  };
